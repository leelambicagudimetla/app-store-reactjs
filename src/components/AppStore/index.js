...
class AppStore extends Component {
  state = {
    tabIdResult: tabsList[0].tabId,
    searchInput: '',
  }

  updateTabItem = tabId => {
    this.setState({tabIdResult: tabId})
  }
  ...
  searchResultFun = event => {
    this.setState({searchInput: event.target.value})
  }

  getFilteredProjects = () => {
    const {tabIdResult} = this.state
    const filteredProjects = appsList.filter(
      each => each.category === tabIdResult,
    )
  ...
  render() {
    const {tabIdResult, searchInput} = this.state
    const filteredProjects = this.getFilteredProjects()
    const result =
      searchInput === ''
        ? filteredProjects
        : filteredProjects.filter(eachSuggestion =>
            eachSuggestion.appName
              .toLowerCase()
